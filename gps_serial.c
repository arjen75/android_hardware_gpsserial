...
/* this implements a GPS hardware library for the serial GPS unit.
...
/* this is the state of our connection to the serial GPS unit */
typedef struct {
    int                     init;
    int                     fd;
...
gps_state_done( GpsState*  s )
{
    ...
    // close connection to the serial GPS unit
    close( s->fd ); s->fd = -1;
    s->init = 0;
}
 ...
 * when started, messages from the serial GPS unit. these are simple NMEA sentences
...
static int serial_gps_set_position_mode(GpsPositionMode mode, GpsPositionRecurrence recurrence, uint32_t min_interval, uint32_t preferred_accuracy, uint32_t preferred_time)
{
    return 0;
}
